  ...
  ; Vanilla C3
	...
	${If} $R0 = 1
	  DetailPrint "Found Creatures 3 (original)."
		...
		make1:
		CreateShortCut "$SMPROGRAMS\openc2e\openc2e - Creatures 3.lnk" "$dir\openc2e.exe" '-d "$plainc3" --autokill'
	${Else}
	  DetailPrint "Could not find Creatures 3 (original)."
		Delete "$SMPROGRAMS\openc2e\openc2e - Creatures 3.lnk"
	${EndIf}
	...
	${If} $R0 = 1
	  DetailPrint "Found Docking Station (original)."
		...
		make2:
		CreateShortCut "$SMPROGRAMS\openc2e\openc2e - Docking Station.lnk" "$dir\openc2e.exe" '-d "$plainds" --autokill'
	${Else}
		DetailPrint "Could not find Docking Station (original)."
	  Delete "$SMPROGRAMS\openc2e\openc2e - Docking Station.lnk"
	${EndIf}
  ...
  !insertmacro IfKeyExists "HKEY_LOCAL_MACHINE" "Software\CyberLife Technology" "Edynn"
	...
	${If} $R0 = 1
	  DetailPrint "Found Edynn."
		...
		make3:
		CreateShortCut "$SMPROGRAMS\openc2e\openc2e - Edynn.lnk" "$dir\openc2e.exe" '-d "$edynn" --autokill'
	${Else}
	  DetailPrint "Could not find Edynn."
	  Delete "$SMPROGRAMS\openc2e\openc2e - Edynn.lnk"
	${EndIf}
	...
	${If} $R0 = 1
	  DetailPrint "Found Creatures 3 (Exodus)."
		...
		make4:
		CreateShortCut "$SMPROGRAMS\openc2e\openc2e - Creatures 3 (Exodus).lnk" "$dir\openc2e.exe" '-d "$exodc3" --autokill'
	${Else}
	  DetailPrint "Could not find Creatures 3 (Exodus)."
	  Delete "$SMPROGRAMS\openc2e\openc2e - Creatures 3 (Exodus).lnk"
	${EndIf}
	...
	${If} $R0 = 1
	  DetailPrint "Found Docking Station (Exodus)."
		...
		make5:
		CreateShortCut "$SMPROGRAMS\openc2e\openc2e - Docking Station (Exodus).lnk" "$dir\openc2e.exe" '-d "$exodds" --autokill'
	${Else}
	  DetailPrint "Could not find Docking Station (Exodus)."
	  Delete "$SMPROGRAMS\openc2e\openc2e - Docking Station (Exodus).lnk"
	${EndIf}
  ...
  !insertmacro IfKeyExists "HKEY_LOCAL_MACHINE" "Software\CyberLife Technology" "Creatures Adventures"
	...
	${If} $R0 = 1
	  DetailPrint "Found Creatures Adventures."
		...
		StrCpy $ca "$ca" -1
		make6:
		CreateShortCut "$SMPROGRAMS\openc2e\openc2e - Creatures Adventures.lnk" "$dir\openc2e.exe" '-d "$ca" -g cv --autokill'
	${Else}
	  DetailPrint "Could not find Creatures Adventures."
	  Delete "$SMPROGRAMS\openc2e\openc2e - Creatures Adventures.lnk"
	${EndIf}
  ...
  !insertmacro IfKeyExists "HKEY_LOCAL_MACHINE" "Software\CyberLife Technology" "Creatures Playground"
	...
	${If} $R0 = 1
	  DetailPrint "Found Creatures Playground."
		...
		StrCpy $cp "$cp" -1
		make7:
		CreateShortCut "$SMPROGRAMS\openc2e\openc2e - Creatures Playground.lnk" "$dir\openc2e.exe" '-d "$cp" -g cv --autokill'
	${Else}
	  DetailPrint "Could not find Creatures Playground."
	  Delete "$SMPROGRAMS\openc2e\openc2e - Creatures Playground.lnk"
	${EndIf}
	...
	${If} $R0 = 1
	  DetailPrint "Found Creatures Village."
		...
		StrCpy $cv "$cv" -1
		make8:
		CreateShortCut "$SMPROGRAMS\openc2e\openc2e - Creatures Village.lnk" "$dir\openc2e.exe" '-d "$cv" -g cv --autokill'
	${Else}
	  DetailPrint "Could not find Creatures Village."
	  Delete "$SMPROGRAMS\openc2e\openc2e - Creatures Village.lnk"
	${EndIf}
	...
	${If} $R0 = 1
	  DetailPrint "Found Creatures 2 (original)."
		...
		make9:
		CreateShortCut "$SMPROGRAMS\openc2e\openc2e - Creatures 2.lnk" "$dir\openc2e.exe" '-d "$plainc2" -g c2 --autokill'
	${Else}
	  DetailPrint "Could not find Creatures 2 (original)."
	  Delete "$SMPROGRAMS\openc2e\openc2e - Creatures 2.lnk"
	${EndIf}
	...
	${If} $R0 = 1
	  DetailPrint "Found Creatures 1 (original)."
		...
		make10:
		CreateShortCut "$SMPROGRAMS\openc2e\openc2e - Creatures 1.lnk" "$dir\openc2e.exe" '-d "$plainc1" -g c1 --autokill'
	${Else}
	  DetailPrint "Could not find Creatures 1 (original)."
	  Delete "$SMPROGRAMS\openc2e\openc2e - Creatures 1.lnk"
	${EndIf}
	...
	${If} $R0 = 1
	  DetailPrint "Found Creatures 2 (The Albian Years)."
		...
		make11:
		CreateShortCut "$SMPROGRAMS\openc2e\openc2e - Creatures 2 (TAY).lnk" "$dir\openc2e.exe" '-d "$tayc2" -g c2 --autokill'
	${Else}
	  DetailPrint "Could not find Creatures 2 (The Albian Years)."
	  Delete "$SMPROGRAMS\openc2e\openc2e - Creatures 2 (TAY).lnk"
	${EndIf}
	...
	${If} $R0 = 1
	  DetailPrint "Found Creatures 1 (The Albian Years)."
		...
		make12:
		CreateShortCut "$SMPROGRAMS\openc2e\openc2e - Creatures 1 (TAY).lnk" "$dir\openc2e.exe" '-d "$tayc1" -g c1 --autokill'
	${Else}
	  DetailPrint "Could not find Creatures 1 (The Albian Years)."
	  Delete "$SMPROGRAMS\openc2e\openc2e - Creatures 1 (TAY).lnk"
	${EndIf}
SectionEnd
