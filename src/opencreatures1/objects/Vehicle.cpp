...
void Vehicle::serialize(SFCContext& ctx, sfc::VehicleV1* veh) {
	...
	if (ctx.is_storing()) {
		...
		veh->x_times_256 = numeric_cast<int32_t>(parts[0].renderable.x() * 256);
		veh->y_times_256 = numeric_cast<int32_t>(parts[0].renderable.y() * 256);
		...
		auto obj_x = parts[0].renderable.x();
		auto obj_y = parts[0].renderable.y();
		...
		if (obj_x != veh_x || obj_y != veh_y) {
			...
			for (auto& cp : parts) {
				cp.renderable.set_position(cp.renderable.x() + diff_x, cp.renderable.y() + diff_y);
			}
		}
		cabin = veh->cabin;
		bump = veh->bump;
