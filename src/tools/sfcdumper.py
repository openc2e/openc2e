...
class CGallery:
	def read(self, f):
		...
		print("gallery read file '" + repr(self.filename) + "' with " + str(self.noframes) + " frame(s) starting at " + str(self.firstsprite))
...
class Entity: # like a compound part?
	def read(self, f):
		...
		print("sprite file: " + repr(self.sprite.filename))
...
class Object:
	...
	def partialread(self, f):
		...
		if self.currentsound[0] != 0:
			print("current sound: " + repr(self.currentsound))
...
#			x = f.read() # TODO
		...
		assert self.mysterybytes == b"\xff\xff\xff\xff\x00"
		self.callbuttonys = []
		for i in range(8):
...
favplacex = read16(f)
favplacey = read16(f)
print("our favourite place is " + repr(favplacename) + ", at (" + str(favplacex) + ", " + str(favplacey) + ")")
