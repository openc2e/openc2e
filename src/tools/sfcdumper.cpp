...
struct CGalleryV1 : MFCObject {
	...
	void deserialize(MFCReader& in) override {
...
struct Vehicle : CompoundObjectV1 {
	...
	void deserialize(MFCReader& in) override {
		CompoundObjectV1::deserialize(in);
...
struct Blackboard : CompoundObjectV1 {
...
struct Lift : Vehicle {
	...
	void deserialize(MFCReader& in) override {
		...
		for (size_t i = 0; i < 8; ++i) {
			...
			floors[i].call_button = in.read_type<CallButton>();
		}
	}
};

struct CallButton : SimpleObjectV1 {
	Lift* lift;
	uint8_t button_id;

	void deserialize(MFCReader& in) override {
		SimpleObjectV1::deserialize(in);
		lift = in.read_type<Lift>();
		button_id = in.read8();
	}
};

struct Scenery : ObjectV1 {
	EntityV1* part;
...
struct Macro : MFCObject {
	...
	void deserialize(MFCReader& in) override {
		...
		fmt::print("macro most_recently_visited_subroutine = {}\n", subroutine_label);
...
SFCFile read_sfc_file(std::istream& in) {
	// set up types
	MFCReader reader(in);
	...
	reader.register_class<Vehicle>("Vehicle", 1);
	reader.register_class<Lift>("Lift", 1);
	reader.register_class<Scenery>("Scenery", 1);
	reader.register_class<Macro>("Macro", 1);
	reader.register_class<Blackboard>("Blackboard", 1);
	reader.register_class<CallButton>("CallButton", 1);
	...
	for (size_t i = 0; i < num_sceneries; ++i) {
		sfc.sceneries.push_back(reader.read_type<Scenery>());
	}
	...
	for (size_t i = 0; i < num_macros; ++i) {
		sfc.macros.push_back(reader.read_type<Macro>());
	}
...
int main(int argc, char** argv) {
	...
	auto input_filename = fs::path(argv[1]);

	std::ifstream in(input_filename, std::ios_base::binary);

	auto sfc = read_sfc_file(in);
