...
Image ToRGB24(const Image &oldimage) {
  if (oldimage.format == if_rgb24) return oldimage;
...
Image StitchBackground(const MultiImage &images) {
    ...
    const unsigned int heightinsprites = [&]{
      if (images.size() == 464) return 8;
      if (images.size() == 928) return 16;
      throw creaturesException("Number of frames should be 464 or 928");
    }();
    const int bytes_per_pixel = [&]{
      if (images[0].format == if_index8) return 1;
      if (images[0].format == if_rgb555 || images[0].format == if_rgb565) return 2;
