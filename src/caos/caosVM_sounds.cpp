...
void c_SNDE(caosVM *vm) {
	...
	valid_agent(vm->targ);
	if (engine.camera->getMetaRoom() != world.map->metaRoomAt(vm->targ->x, vm->targ->y) || !agentOnCamera(vm->targ)) return; // TODO: is it correct behaviour for only onscreen agents to play?
...
void c_PLDS(caosVM *vm) {
	...
	valid_agent(vm->targ);
	if (engine.camera->getMetaRoom() != world.map->metaRoomAt(vm->targ->x, vm->targ->y)) return; // TODO: needs better check ;)
