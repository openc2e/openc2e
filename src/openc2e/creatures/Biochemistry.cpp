...
void c2eCreature::adjustChemical(unsigned char id, float value) {
	...
	if (chemicals[id] < 0.0f)
		chemicals[id] = 0.0f;
	else if (chemicals[id] > 1.0f)
		chemicals[id] = 1.0f;
}
...
unsigned int oldCreature::calculateTickMask(unsigned char rate) {
	if (rate < 7)
		...
		return (1 << ((unsigned int)rate - 7)) - 1;
}
...
void c1Creature::tickBiochemistry() {
	...
	if ((ticks % 5) != 0)
		return; // TODO: what is the correct tick rate?
...
void oldCreature::tickBiochemistry() {
	...
	if (halflives) { // TODO: correct?
		for (unsigned int i = 0; i < 256; i++) {
			...
			if ((biochemticks & calculateTickMask(rate)) != 0)
				continue;
...
void c2Organ::tick() {
	...
	// *** tick receptors
	if (clockratereceptors != 0)
		clockrate = 0;
	if (repairratereceptors != 0)
		repairrate = 0;
	if (injuryreceptors != 0)
		injurytoapply = 0;
...
void c2eOrgan::tick() {
	...
	// *** tick receptors
	for (auto& reaction : reactions)
		reaction->receptors = 0;
	clockratereceptors = 0;
	repairratereceptors = 0;
	injuryreceptors = 0;
	...
	for (auto& reaction : reactions)
		if (reaction->receptors > 0)
			reaction->rate /= reaction->receptors;
	if (clockratereceptors > 0)
		clockrate /= clockratereceptors;
	if (repairratereceptors > 0)
		repairrate /= repairratereceptors;
	if (injuryreceptors > 0)
		injurytoapply /= injuryreceptors;
...
void c2Organ::processReaction(c2Reaction& d) {
	...
	// if the tickmask doesn't want us to change things this tick, don't!
	if ((biochemticks & parent->calculateTickMask(rate)) != 0)
		return;
