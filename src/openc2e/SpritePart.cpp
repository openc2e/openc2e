...
SpritePart::SpritePart(Agent* p, unsigned int _id, std::string spritefile, unsigned int fimg,
	...
	if (sprite->numframes() <= firstimg) {
		if (engine.gametype == "cv") {
			...
			throw Exception(fmt::format("Failed to create sprite part: first sprite {} is beyond {} sprite(s) in file {}", firstimg, sprite->numframes(), sprite->getName()));
		}
	}
}
...
void SpritePart::partRender(RenderTarget* renderer, int xoffset, int yoffset) {
	...
	if (getCurrentSprite() >= getSprite()->numframes()) {
		if (engine.version == 2) {
			...
			throw Exception(fmt::format("pose to be rendered {} (firstimg {}, base {}) was past end of sprite file '{}' ({} sprites)",
...
void SpritePart::setPose(unsigned int p) {
	if (firstimg + base + p >= getSprite()->numframes()) {
		if (engine.version == 2) {
			...
			throw Exception(fmt::format("new pose {} (firstimg {}, base {}) was past end of sprite file '{}' ({} sprites)",
...
void SpritePart::changeSprite(std::string spritefile, unsigned int fimg) {
	...
	THROW_IFNOT(spr->numframes() > fimg);
	firstimg = fimg;
	changeSprite(spr);
}
