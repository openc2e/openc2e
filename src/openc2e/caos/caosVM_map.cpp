...
void v_GRID(caosVM* vm) {
	...
	THROW_IFNOT(direction >= 0);
	THROW_IFNOT(direction <= 3);
	VM_PARAM_VALIDAGENT(agent)
...
void c_EMIT(caosVM* vm) {
	...
	THROW_IFNOT((0 <= caindex && caindex <= 19) || caindex == -1);
	valid_agent(vm->targ);
...
void c_CACL(caosVM* vm) {
	...
	THROW_IFNOT(0 <= species && species <= 65535);
	VM_PARAM_INTEGER(genus)
	THROW_IFNOT(0 <= genus && genus <= 255);
	VM_PARAM_INTEGER(family)
	THROW_IFNOT(0 <= family && family <= 255);
...
void v_GRND(caosVM* vm) {
	...
	THROW_IFNOT(index >= 0 && (unsigned int)index < world.groundlevels.size());
...
void v_ROOM_c1(caosVM* vm) {
	...
	THROW_IFNOT(data >= 0 && data <= (engine.version == 1 ? 4 : 19));
	VM_PARAM_INTEGER(roomno)
...
void c_SSFC(caosVM* vm) {
	...
	THROW_IFNOT(coordcount >= 0); // this should never happen unless the parser breaks or we load a bad savefile
	...
	for (int i = 0; i < coordcount; i++) {
		...
		THROW_IFNOT((unsigned int)x <= (r->x_right - r->x_left));
		THROW_IFNOT((unsigned int)y <= (r->y_left_floor - r->y_left_ceiling));
