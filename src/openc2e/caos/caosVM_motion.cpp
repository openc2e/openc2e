...
void c_MVSF(caosVM* vm) {
	...
	if (!vm->targ->tryMoveToPlaceAround(x, y))
		throw Exception(fmt::format("MVSF failed to find a safe place around ({}, {})", x, y));
}
...
void c_FRIC(caosVM* vm) {
	...
	THROW_IFNOT(friction >= 0);
	THROW_IFNOT(friction <= 100);
...
void c_ADMP(caosVM* vm) {
	...
	THROW_IFNOT(angulardamping >= 0.0f);
	THROW_IFNOT(angulardamping <= 1.0f);
...
void c_FDMP(caosVM* vm) {
	...
	THROW_IFNOT(forwarddamping >= 0.0f);
	THROW_IFNOT(forwarddamping <= 1.0f);
...
void c_SDMP(caosVM* vm) {
	...
	THROW_IFNOT(sidewaysdamping >= 0.0f);
	THROW_IFNOT(sidewaysdamping <= 1.0f);
...
void c_ROTN(caosVM* vm) {
	...
	THROW_IFNOT(rotations > 0);
	VM_PARAM_INTEGER(sprites)
	THROW_IFNOT(sprites > 0);
