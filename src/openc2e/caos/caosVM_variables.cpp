...
void c_ADDS(caosVM* vm) {
	...
	THROW_IFNOT(variable->hasString());
...
void v_TYPE(caosVM* vm) {
	...
	if (value.hasInt())
		...
		throw Exception("caosValue confused us terribly in TYPE");
}
...
void v_SQRT(caosVM* vm) {
	...
	THROW_IFNOT(x >= 0); // no imaginary numbers for you!
...
void v_CHAR(caosVM* vm) {
	...
	THROW_IFNOT(index >= 1);
	THROW_IFNOT(str.size() >= (unsigned int)index);
...
void c_CHAR(caosVM* vm) {
	...
	THROW_IFNOT(0 <= character && character <= 255);

	THROW_IFNOT(str->hasString());
	std::string mystr = str->getString();
	THROW_IFNOT(index >= 1);
	THROW_IFNOT(mystr.size() >= (unsigned int)index);
...
void v_READ(caosVM* vm) {
	...
	THROW_IFNOT(catalogue.hasTag(tag));
	const std::vector<std::string>& t = catalogue.getTag(tag);
	THROW_IFNOT(offset >= 0);
	THROW_IFNOT((unsigned int)offset < t.size());
...
void v_CATX(caosVM* vm) {
	...
	THROW_IFNOT(catalogue.hasTag("Agent Categories"));
...
void v_WILD(caosVM* vm) {
	...
	THROW_IFNOT(searchstring.size()); // check we found a tag

	const std::vector<std::string>& t = catalogue.getTag(searchstring); // retrieve tag
	THROW_IFNOT(offset >= 0);
	THROW_IFNOT((unsigned int)offset < t.size()); // check the offset is useful for the tag we found
...
void c_NAMN(caosVM* vm) {
	...
	if (previous->hasString() && previous->getString().empty()) {
		...
		THROW_IFNOT(i != vm->targ->name_variables.end()); // TODO: this probably isn't correct behaviour
		i++;
		if (i == vm->targ->name_variables.end())
			previous->setString("");
		else
