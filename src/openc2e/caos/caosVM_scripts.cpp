...
void c_WAIT(caosVM* vm) {
	...
	THROW_IFNOT(ticks >= 0); // TODO: is this right?
...
void v_SORQ(caosVM* vm) {
	VM_PARAM_INTEGER(event)
	THROW_IFNOT(event >= 0 && event <= 65535);
	VM_PARAM_INTEGER(species)
	THROW_IFNOT(species >= 0 && species <= 65535);
	VM_PARAM_INTEGER(genus)
	THROW_IFNOT(genus >= 0 && genus <= 255);
	VM_PARAM_INTEGER(family)
	THROW_IFNOT(family >= 0 && family <= 255);
