...
class badParamException : public caosException {
  public:
	badParamException()
		: caosException("parameter type mismatch") {}
};
...
class caosVM {
  ...
  public:
	...
	void setVariables(caosValue& one, caosValue& two) {
		_p_[0] = one;
		_p_[1] = two;
	}
...
class notEnoughParamsException : public caosException {
  public:
	notEnoughParamsException()
		: caosException("Not enough parameters") {}
};

class invalidAgentException : public caosException {
  public:
	invalidAgentException()
		: caosException("Invalid agent handle") {}
	invalidAgentException(const char* s)
		: caosException(s) {}
	invalidAgentException(const std::string& s)
		: caosException(s) {}
};
...
template <typename T>
auto vmparamhelper(const caosValue& val);
template <>
inline auto vmparamhelper<caosValue>(const caosValue& val) {
	return val;
}
template <>
inline auto vmparamhelper<std::string>(const caosValue& val) {
	return val.getString();
}
template <>
inline auto vmparamhelper<int>(const caosValue& val) {
	return val.getInt();
}
template <>
inline auto vmparamhelper<float>(const caosValue& val) {
	return val.getFloat();
}
template <>
...
template <>
inline auto vmparamhelper<AgentRef>(const caosValue& val) {
	return val.getAgent();
}
...
#define VM_PARAM_VARIABLE(name) \
	...
	caosValue* const name = &vm__lval_##name.value;
#define VM_PARAM_DECIMAL(name) \
	caosValue name; \
	{ \
		VM_STACK_CHECK(vm); \
		caosValue __x = vm->valueStack.back(); \
