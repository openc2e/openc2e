...
TEST(File, file_write_read_flush_read) {
	FileWriter w(fname());
	w.write("hello");
...
TEST(File, file_read_nonexistent_errors) {
	EXPECT_THROW(FileReader f(fname()), IOException);
}

TEST(File, file_read_parts) {
	FileWriter(fname()).write("hello world");
...
TEST(File, file_seek_relative_backward) {
	FileWriter(fname()).write("hello world");
	FileReader f(fname());
	(void)f.read_to_end();
	f.seek_relative(-5);
...
TEST(File, file_seek_relative_forward) {
	FileWriter(fname()).write("hello world");
	FileReader f(fname());
	f.seek_relative(6);
	auto contents = f.read_to_end();
...
TEST(File, file_read_to_end_tell) {
	FileWriter(fname()).write("hello world");
	FileReader f(fname());
	(void)f.read_to_end();
	EXPECT_EQ(f.tell(), 11);
...
TEST(File, file_read_past_end_errors) {
	FileWriter(fname()).write("hello");
	FileReader f(fname());
...
TEST(File, file_seek_and_read_past_end_errors) {
	FileWriter(fname()).write("hello");
	FileReader f(fname());
	f.seek_absolute(11);
...
TEST(File, file_read_parts_and_has_data_left_true) {
	FileWriter(fname()).write("hello world");
	FileReader f(fname());
...
TEST(File, file_read_parts_and_has_data_left_false) {
	FileWriter(fname()).write("hello");
	FileReader f(fname());
	std::vector<uint8_t> contents(5);
	f.read(contents);
...
TEST(File, file_read_to_end_and_has_data_left_false) {
	FileWriter(fname()).write("hello");
	FileReader f(fname());
	(void)f.read_to_end();
	EXPECT_FALSE(f.has_data_left());
...
TEST(File, file_seek_and_has_data_left_false) {
	FileWriter(fname()).write("hello");
	FileReader f(fname());
	f.seek_absolute(5);
	EXPECT_FALSE(f.has_data_left());
...
TEST(File, file_with_unicode_name) {
