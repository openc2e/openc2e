...
mechanisms for adding tests are provided.

The older :command:`gtest_add_tests` function attempts to identify tests by
scanning source files.  Although this is generally effective, it may have
difficulty discovering tests if they are declared in an unusual manner, or
advanced features such as parameterized tests are in use.  Additionally, it
requires re-running CMake if the source files change in order to detect new or
removed tests.

By contrast, the new :command:`gtest_discover_tests` function sets up
build-time targets that execute the compiled test executables with the
``--gtest_list_tests`` argument in order to obtain the list of available tests.
Although this does not support cross-compiling, it is otherwise more robust,
and does not require re-running CMake in order to discover added or removed
tests within a test executable.  Each such target is added to an automatically
created master target ``gtest_discover_tests``.  This target is included in
``ALL``, but you may wish to register it as a dependency of some other
target(s) in your project::

  add_dependencies(my_tests gtest_discover_tests)

Both commands are intended to replace use of :command:`add_test` to register
tests, and will create a separate CTest test for each test case.  Note that
this is in some cases less efficient, as common set-up and tear-down logic
cannot be shared by multiple test cases executing in the same instance.
However, this is usually preferred as it provides more fine-grained pass/fail
information to CTest.  By default, the CTest test name is the same as the
Google Test name (i.e. ``suite.testcase``); see also ``TEST_PREFIX`` and
``TEST_SUFFIX``.
...
.. command:: gtest_discover_tests
  ...
  ``TEST_PREFIX prefix``
    Allows the specified ``prefix`` to be prepended to the name of each
    discovered test case.  This can be useful when the same source files are
    being used in multiple calls to ``gtest_discover_test()`` but with
...
function(gtest_discover_tests TARGET)
  ...
  add_custom_target(gtest_discover_${TARGET}_tests DEPENDS "${ctest_file}")
  add_dependencies(
    ${GOOGLETEST_DISCOVER_TESTS_TARGET}
    gtest_discover_${TARGET}_tests
  )
...
if(NOT DEFINED GOOGLETEST_DISCOVER_TESTS_TARGET)
  set(GOOGLETEST_DISCOVER_TESTS_TARGET gtest_discover_tests)
endif()

if(NOT TARGET ${GOOGLETEST_DISCOVER_TESTS_TARGET})
  add_custom_target(${GOOGLETEST_DISCOVER_TESTS_TARGET} ALL)
endif()